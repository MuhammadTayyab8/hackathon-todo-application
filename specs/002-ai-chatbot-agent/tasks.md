...
- [ ] T001 Install OpenAI Agents SDK in backend/requirements.txt (add openai-agents==1.0.0)
- [ ] T002 [P] Install OpenAI client library in backend/requirements.txt (add openai>=1.0.0)
- [ ] T003 [P] Install tiktoken for token counting in backend/requirements.txt (add tiktoken>=0.5.0)
- [ ] T004 [P] Create agents module directory structure at backend/src/agents/__init__.py
- [ ] T005 Verify MCP server from Sub-Phase 1 is operational (run backend/mcp_server_main.py and test tools)
- [ ] T006 Configure OPENROUTER_API_KEY environment variable in backend/.env
...
- [ ] T007 [P] Create Conversation SQLModel in backend/src/models/conversation.py with fields (id, user_id, title, created_at, updated_at) and Message relationship
- [ ] T008 [P] Create Message SQLModel in backend/src/models/message.py with fields (id, conversation_id, role, content, tool_calls, created_at) and Conversation relationship
- [ ] T009 [P] Create MessageRole enum in backend/src/models/message.py (USER, ASSISTANT, SYSTEM)
- [ ] T010 Generate Alembic migration for conversation and message tables in backend/alembic/versions/
- [ ] T011 Apply database migration to create conversation and message tables (run alembic upgrade head)

### Agent Configuration

- [ ] T012 Configure OpenRouter client with Gemini 2.5 Flash in backend/src/agents/task_agent.py (base_url="https://openrouter.ai/api/v1", model="google/gemini-2.0-flash-exp:free")
- [ ] T013 Define agent system instructions in backend/src/agents/task_agent.py (task management assistant with tool guidelines, confirmation rules, NL parsing examples)

### MCP Tool Wrappers

- [ ] T014 [P] Create add_task MCP tool wrapper in backend/src/agents/mcp_tools.py using @function_tool decorator with JWT token parameter
- [ ] T015 [P] Create list_tasks MCP tool wrapper in backend/src/agents/mcp_tools.py using @function_tool decorator with status filter and JWT token
- [ ] T016 [P] Create complete_task MCP tool wrapper in backend/src/agents/mcp_tools.py using @function_tool decorator with task_id and JWT token
- [ ] T017 [P] Create update_task MCP tool wrapper in backend/src/agents/mcp_tools.py using @function_tool decorator with task_id, fields, and JWT token
- [ ] T018 [P] Create delete_task MCP tool wrapper in backend/src/agents/mcp_tools.py using @function_tool decorator with task_id and JWT token
- [ ] T019 Configure MCP client connection parameters in backend/src/agents/mcp_tools.py (StdioServerParameters with command="python", args=["backend/mcp_server_main.py"])

### Database Operations

- [ ] T020 [P] Implement create_conversation function in backend/src/services/chat_service.py (async, returns Conversation with user_id)
- [ ] T021 [P] Implement get_conversation function in backend/src/services/chat_service.py (async, with user isolation check)
- [ ] T022 [P] Implement create_message function in backend/src/services/chat_service.py (async, stores Message with conversation_id, role, content, tool_calls)
- [ ] T023 [P] Implement get_conversation_messages function in backend/src/services/chat_service.py (async, retrieves messages ordered by created_at)
...
- [ ] T024 [US1] Create agent initialization function in backend/src/agents/task_agent.py (create_task_agent with jwt_token parameter, returns Agent with 5 MCP tools)
- [ ] T025 [US1] Implement basic agent runner in backend/src/agents/task_agent.py (run_agent function that takes message and returns response)
- [ ] T026 [US1] Implement process_chat_message function in backend/src/services/chat_service.py (stateless flow: create/get conversation, store user message, run agent, store assistant response, return result)
- [ ] T027 [US1] Create ChatRequest Pydantic model in backend/src/routes/chat.py (message: str, conversation_id: Optional[UUID])
- [ ] T028 [US1] Create ChatResponse Pydantic model in backend/src/routes/chat.py (conversation_id: UUID, message: str, created_at: datetime)
- [ ] T029 [US1] Implement POST /api/{user_id}/chat endpoint in backend/src/routes/chat.py (verify JWT, validate request, call chat service, return response)
- [ ] T030 [US1] Add JWT verification dependency in backend/src/routes/chat.py (verify user_id matches JWT token)
- [ ] T031 [US1] Implement error handling in backend/src/routes/chat.py (401 for invalid JWT, 400 for bad request, 500 for server errors)
- [ ] T032 [US1] Register chat router in backend/main.py (app.include_router for /api/{user_id}/chat)
...
- [ ] T033 [US2] Implement token counting function in backend/src/agents/history_manager.py (count_tokens using tiktoken with gpt-4 encoding)
- [ ] T034 [US2] Implement history summarization function in backend/src/agents/history_manager.py (summarize_history that keeps last 3 messages, summarizes older messages using Gemini, returns condensed history)
- [ ] T035 [US2] Implement fetch_conversation_history function in backend/src/agents/history_manager.py (retrieves messages from DB, converts to message array format)
- [ ] T036 [US2] Update process_chat_message in backend/src/services/chat_service.py to fetch history if conversation_id provided
- [ ] T037 [US2] Update process_chat_message in backend/src/services/chat_service.py to count tokens and trigger summarization if >600 tokens
- [ ] T038 [US2] Update agent runner in backend/src/agents/task_agent.py to accept history context and build message array with system instructions + history + new message
- [ ] T039 [US2] Update Conversation model in backend/src/models/conversation.py to auto-generate title from first user message (add title generation logic)
...
- [ ] T040 [US3] Add confirmation guidelines to agent system instructions in backend/src/agents/task_agent.py (detect destructive operations, ask for confirmation, wait for response, execute or cancel)
- [ ] T041 [US3] Update agent instructions in backend/src/agents/task_agent.py with confirmation examples (delete all tasks, bulk complete, etc.)
- [ ] T042 [US3] Test confirmation workflow with destructive operations (verify agent asks before deleting, respects user response)
...
- [ ] T043 [US4] Add tool chaining guidelines to agent system instructions in backend/src/agents/task_agent.py (chain tools in logical order, handle partial failures, show all results)
- [ ] T044 [US4] Update agent runner in backend/src/agents/task_agent.py to support multiple tool calls per message (enforce 10 tool call limit)
- [ ] T045 [US4] Implement partial failure handling in backend/src/agents/task_agent.py (if tool chain fails midway, communicate what succeeded and what failed)
- [ ] T046 [US4] Test tool chaining scenarios (list then delete, create then list, find then update, etc.)
...
- [ ] T047 [P] Add comprehensive error logging in backend/src/services/chat_service.py (log all errors with context)
- [ ] T048 [P] Add request/response logging in backend/src/routes/chat.py (log user_id, message length, response time)
- [ ] T049 [P] Implement rate limiting for chat endpoint in backend/src/routes/chat.py (60 requests per minute per user)
- [ ] T050 [P] Add input validation in backend/src/routes/chat.py (message max length 2000 chars, conversation_id format validation)
- [ ] T051 [P] Sanitize error messages in backend/src/routes/chat.py (never expose stack traces or internal details to users)
- [ ] T052 [P] Add performance monitoring in backend/src/services/chat_service.py (track agent response time, token count, tool call count)
- [ ] T053 Update API documentation in backend/src/routes/chat.py (add OpenAPI docstrings for chat endpoint)
- [ ] T054 Run quickstart.md validation (follow all test scenarios in specs/002-ai-chatbot-agent/quickstart.md)
- [ ] T055 Verify all constitution checks pass (JWT verification, user scoping, no secrets on frontend)

...
- [X] T044 [US2] Implement POST /api/auth/signin endpoint that validates UserSignIn credentials in backend/src/api/routes/auth.py using python-fastapi skill
- [X] T045 [US2] Call authenticate_user() function in POST /api/auth/signin to verify password in backend/src/api/routes/auth.py using python-fastapi skill
- [X] T046 [US2] Generate and return JWT token on successful sign in with AuthResponse in backend/src/api/routes/auth.py using python-fastapi skill
- [X] T047 [US2] Return 401 Unauthorized error for invalid credentials without revealing which field is incorrect in POST /api/auth/signin using python-fastapi skill

#### Backend: Authenticate User Service

- [X] T048 [US2] Create authenticate_user() async function that finds user by email and verifies password in backend/src/services/auth_service.py using sqlmodel skill
- [X] T049 [US2] Return None if user not found or password invalid in authenticate_user() using sqlmodel skill

#### Frontend: Sign In Form

- [X] T050 [US2] Create SignInForm client component with email, password state fields in frontend/src/components/auth/SignInForm.tsx using frontend-agent skill
- [X] T051 [US2] Implement form submission handler calling authClient.signIn.email() in SignInForm using frontend-agent skill
- [X] T052 [US2] Add error state and display for sign in failures in SignInForm using frontend-agent skill
- [X] T053 [US2] Add loading state and disable submit button during sign in in SignInForm using frontend-agent skill
- [X] T054 [US2] Redirect to home page (/) on successful sign in in SignInForm using frontend-agent skill

#### Frontend: Sign In Page

- [X] T055 [US2] Create /app/(auth)/signin/page.tsx server component that renders SignInForm using frontend-agent skill
- [X] T056 [US2] Add navigation link to /signup page in /app/(auth)/signin/page.tsx using frontend-agent skill
...
- [X] T057 [US3] Implement POST /api/auth/signout endpoint that clears session in backend/src/api/routes/auth.py using python-fastapi skill
- [X] T058 [US3] Add BearerAuth security requirement to POST /api/auth/signout endpoint using python-fastapi skill
- [X] T059 [US3] Return 200 OK with success message for sign out in POST /api/auth/signout using python-fastapi skill

#### Backend: Get Current User Endpoint

- [X] T060 [US3] Implement GET /api/auth/me endpoint that returns current user info in backend/src/api/routes/auth.py using python-fastapi skill
- [X] T061 [US3] Add BearerAuth security requirement to GET /api/auth/me endpoint using python-fastapi skill
- [X] T062 [US3] Extract user_id from request.state and return user info in GET /api/auth/me using python-fastapi skill

#### Frontend: API Client Integration

- [X] T063 [US3] Implement 401 error handler in ApiClient that redirects to /signin using frontend-agent skill
- [X] T064 [US3] Update ApiClient to retrieve JWT token from Better Auth session before each request using frontend-agent skill
- [X] T065 [US3] Add Authorization header with Bearer token format to all API requests in ApiClient using frontend-agent skill
...
- [X] T066 [P] Test complete auth flow: signup → signin → API access using manual testing as outlined in quickstart.md
- [X] T067 [P] Verify password complexity validation meets requirements (12+ chars, uppercase, lowercase, number, special char) in backend/src/services/auth_service.py
- [X] T068 [P] Verify JWT middleware correctly validates tokens and extracts user_id in backend/src/api/middleware/jwt_middleware.py
- [X] T069 [P] Verify Better Auth configuration matches backend JWT secret (BETTER_AUTH_SECRET) in frontend/src/lib/auth.ts and backend/.env
- [X] T070 [P] Verify CORS configuration allows frontend to communicate with backend in backend/main.py
- [X] T071 [P] Test API endpoints with curl commands from quickstart.md (health check, signup, signin)
- [X] T072 [P] Verify frontend forms display appropriate error messages for all failure scenarios
- [X] T073 [P] Verify frontend successfully redirects to home page after successful sign in/sign up
- [X] T074 [P] Verify 7-day JWT token expiry is configured correctly in both frontend and backend
- [X] T075 [P] Verify all passwords are hashed with bcrypt and never stored in plaintext
- [X] T076 [P] Verify unique email and username constraints are enforced by database and API
